...
# from: https://github.com/newrelic/k8s-webhook-cert-manager/blob/master/generate_certificate.sh
...
while true; do
  echo "INFO: Trying to patch webhook adding the caBundle."
  if kubectl patch "ValidatingWebhookConfiguration" "${webhook}" --type='json' -p "[{'op': 'add', 'path': '/webhooks/0/clientConfig/caBundle', 'value':'${caBundle}'}]"; then
      break
  fi
